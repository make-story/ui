...
// dom
YSM.$ = function() {
	return new YSM.dom.Dom(Array.prototype.slice.call(arguments));
};

YSM.dom.Dom = function(arr) {
	...
	*/
	...
	live: function(selector, events, handlers, capture) {
		this.each(function() {
			var callback = function(e) {
				...
				YSM.$(this).find(selector).each(function() {
	...
	off: (function() {
		...
		var func = function(events) {
			...
			// 이벤트 해제
			if(YSM.cache.event[key]) {
				cache = YSM.cache.event[key];
	...
	html: function(parameter) {
			...
			"child": [
				{
					...
					"callback": function(element) { // element 만든 후 작업
						//YSM.module.Slide.call({'instance': this.data.instance}, {'element': element});
					},
		...
		*/
		...
		function html(child, parent) {
			...
			for(index in child) {
				element = YSM.$(document.createElement(child[index].tag));
				// attr
				if(child[index].attr) {
		...
		// 생성
		if(parameter.child) {
			YSM.$(parameter.area || 'body').append(html(parameter.child));
		}
