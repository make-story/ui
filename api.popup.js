...
;(function(factory, global) {
	...
	var Popup = function() {
		...
		// settings (팝업 종류별로 사용하는 공통 설정값)
		that.settings = {
			'story': {
				'gap': 20, // story 팝업간 차이
				'snap': 10, // snap 을 발생시키도록하는 element와 element 간의 간격
	...
	Popup.prototype = {
		...
		getInstance: function(parameter) {
			...
			if(that['instance'][key] && that['instance'][key]['element'] && typeof that['instance'][key]['element']['container'] === 'object') {
				return that['instance'][key];
			}else {
		...
		setFolderElement: function(instance) {
			...
			var width = that['settings']['folder']['width'] + api['env']['browser']['scrollbar'];
			...
			container.innerHTML = '\
				...
				<div style="max-height: 200px; background-color: rgb(248, 249, 250); border-top: 2px solid rgb(232, 233, 234); border-bottom: 2px solid rgb(232, 233, 234); overflow-x: hidden; overflow-y: auto;">\
					<div id="' + keys['grid'] + '" data-type="content" style="margin: 0 auto; width: 300px;">\
						<!-- grid 로딩 메시지 //-->\
			...
			api.dom(close_button).on(api['env']['event']['down'], function(e) {
				var event = e || window.event;
				// 현재 이벤트가 상위로 전파되지 않도록 중단한다.
				if(event.stopPropagation) { 
					event.stopPropagation();
				...
				*/
				xhr({
					...
					'progressDownload': function(progress) {
						...
						if(typeof instance['element']['bar'] === 'object') {
							instance['element']['bar'].style.display = 'block';
			...
			if(api['env']['device'] === 'mobile') {
				...
				return function(instance) {
					...
					api.dom(button_close).on(api['env']['event']['down'], function(e) {
						...
						iframe.onload = null;
						//container.style.display = 'none';
						that.setClose({'key': instance['key'], 'remove': true});
					});
				...
				return function(instance) {
					...
					api.dom(header).on(api['env']['event']['down'], function(e) {
						...
						that['settings']['story']['left'] = 0;
						that['settings']['story']['top'] = 0;
						...
						api.dom(window).on(api['env']['event']['move'] + '.EVENT_MOUSEMOVE_popup_story_move', function(e) {
							...
							// snap 영역 검사
							var i, max;
							var interval = that['settings']['story']['snap']; // snap 을 발생시키도록하는 element와 element 간의 간격
					...
					(function setResize() {
						...
						api.dom(right_resize).on(api['env']['event']['down'], function(e) {
							...
							setMousePositionOn(function() {
								...
								if(0 <= left && that['settings']['story']['min_width'] <= left) {
						...
						api.dom(bottom_resize).on(api['env']['event']['down'], function(e) {
							...
							setMousePositionOn(function() {
								...
								if(0 <= top && that['settings']['story']['min_height'] <= top) {
		...
		setElementWidthHeight: function(parameter) {
			...
			switch(form) {
				case 'bunch':
					instance['element']['container'].style.width = instance['width'] = '300px';
					instance['element']['container'].style.height = instance['height'] = '300px';
					break;
				case 'story':
					if(api['env']['device'] === 'mobile') {
						// 모바일에서의 style
						instance['element']['container'].style.width = instance['width'] = (size['window']['width'] - api['env']['browser']['scrollbar']) + 'px';
						...
						instance['element']['container'].style.width = instance['width'] = (window.localStorage.getItem((instance['form'] + instance['key'] + 'width')) || that['settings']['story']['min_width']) + 'px';
						instance['element']['container'].style.height = instance['height'] = (window.localStorage.getItem((instance['form'] + instance['key'] + 'height')) || that['settings']['story']['min_height']) + 'px';
			...
			switch(form) {
				...
				case 'story':
					if(api['env']['check']['mobile']) {
						...
						if((that['settings']['story']['gap'] * 5) < that['settings']['story']['left'] || (that['settings']['story']['gap'] * 5) < that['settings']['story']['top']) {
							that['settings']['story']['left'] = 0;
							that['settings']['story']['top'] = 0;
						}
						that['settings']['story']['left'] += that['settings']['story']['gap'];
						that['settings']['story']['top'] += that['settings']['story']['gap'];
						instance['element']['container'].style.left = that['settings']['story']['left'] + 'px';
						instance['element']['container'].style.top = that['settings']['story']['top'] + 'px';
					}
					break;
				case 'layer':
		...
		setOpen: function(parameter) {
			...
			var setShow = function() {
				if((!that['action'][form][key] || typeof that['action'][form][key] != 'object') && !setCreate()) {
					console.log('[오류!] element setCreate');
					return false;
				}

				// zindex
				instance['element']['container'].style.zIndex = (that.zindex += 1);
					...
					case 'bunch':
						...
						api.animationQueue({'element': that['element']['bunch'], 'animation': 'animation_moveToBottomBounce', 'complete': function() {
							...
							for(key in that['action']['story']) {
								//title = instance['element']['title'].textContent;
								title = 'test';
								//
								fragment = document.createDocumentFragment();
								//
								...
								(function(d, b, k) {
									api.dom(b).on(api['env']['event']['down'], function(e) {
										var event = e || window.event;
										that.setClose({'key': k, 'remove': true});
		...
		setClose: function(parameter) {
			...
			var key = parameter['key'];
			var callback = parameter['callback'];
			var remove = typeof parameter['remove'] === 'boolean' ? parameter['remove'] : false;
			...
			var setRemove = function() {
				...
				// 
				return instance['element']['container'].parentNode.removeChild(instance['element']['container']) && delete that['instance'][key] && delete that['action'][form][key];
			};
				...
				switch(form) {
					case 'bunch':
						api.animationQueue({'element': that['element']['bunch'], 'animation': 'animation_moveToTop', 'complete': function() {
							...
							//
							instance['element']['container'].style.display = 'none';
							// callback
							if(typeof callback === 'function') {
								callback.call(that);
							}
			...
			if(0 <= (that['settings']['story']['left'] - that['settings']['story']['gap'])) {
				that['settings']['story']['left'] -= that['settings']['story']['gap'];
			}
			if(0 <= (that['settings']['story']['top'] - that['settings']['story']['gap'])) {
				that['settings']['story']['top'] -= that['settings']['story']['gap'];
			}
		...
		setAllClose: function(parameter) {
			...
			for(index in form) {
				if(that['action'][form[index]]) {
					for(key in that['action'][form[index]]) {
						that.setClose({'key': key, 'remove': true});
					}
				}
			}
		}
