...
let Parse = function Parse(template='', context, parent) {
    ...
    let match_tag;
    let match_tag_open, match_tag_close;
    let match_tag_name;
    let match_close;
    let tag; // {{tag}}
    let code; // 일반 html code
    while(this.template !== '') {
...
let render = function render(parse, contents={}/*변수에 할당(파싱된 트리에 contents값 설정)할 json 데이터*/) {
    /*
...
// 3. 프린팅 - html 반환
let paint = function paint(render) {
    let i, max;
    let codes = [];
...
template.parse = function(template) {
    let parse = cache[template]; // 기존 파싱되었던 템플릿인지 확인
    if(!parse) { 
...
template.render = function(template, contents) { // 사용성이 별로 없지만, 흐름확인을 위해 public 
    let parse = this.parse(template);
    return render(parse, contents);
};
template.paint = function(template, contents) {
    let render = this.render(template, contents);
    return paint(render);
};
if(typeof window === 'object' && window.document && window.document.createDocumentFragment) {
    template.fragment = function(template, contents) { // fragment 에 html 삽입 후 반환
        let paint = this.paint(template, contents);
